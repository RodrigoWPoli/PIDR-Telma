...
if "consecutive_counts"  not in st.session_state: st.session_state.consecutive_counts = {"Alert": 0, "Alarm": 0}
if "confirmed_state"     not in st.session_state: st.session_state.confirmed_state = "Healthy"
if "collection_process"  not in st.session_state: st.session_state.collection_process = None
if "collection_log"      not in st.session_state: st.session_state.collection_log = []
if "ontology_log"        not in st.session_state: st.session_state.ontology_log = []
if "refresh_interval"    not in st.session_state: st.session_state.refresh_interval = REFRESH_INTERVAL
    ...
    if is_running:
        ...
        if st.button("Stop collection", use_container_width=True):
            ...
            st.session_state.collection_log.insert(
                0, f"{datetime.now().strftime('%H:%M:%S')} — stopped")
            st.rerun()
    else:
        if proc is not None:
            exit_code = proc.poll()
            if exit_code != 0:
                st.warning("Collection stopped unexpectedly.", icon="⚠️")
            st.session_state.collection_log.insert(
        ...
        if st.button("Start collection", use_container_width=True,
            ...
            import os, sys
            script = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data_collection.py")
        ...
        try:
            ...
            if vals:
                ...
                st.session_state.ontology_log.insert(
                    0, f"{datetime.now().strftime('%H:%M:%S')} — updated (Otr_acc={otr:.1f})")
...
# ── Live content ───────────────────────────────────────────────────────────────
# run_every is read here (outside the fragment) so that changing the sidebar
# slider — which triggers a full page rerun — picks up the new interval.
...
def live_dashboard():
    ...
    with tab_monitor:
        ...
        with left:
            ...
            if history:
                ...
                def bar_colors(vals, n=HYSTERESIS_N):
                    colors, ar, alr, confirmed = [], 0, 0, "Healthy"
                    for v in vals:
                        if v > ALARM_THRESHOLD:
                            ar += 1; alr = 0
                            if ar >= n: confirmed = "Alarm"
                        elif v > ALERT_THRESHOLD:
                            alr += 1; ar = 0
                            if alr >= n: confirmed = "Alert"
                        else:
                            ar = alr = 0; confirmed = "Healthy"
                        colors.append({"Alarm": "#F09595", "Alert": "#FAC775"}.get(confirmed, "#85B7EB"))
                    return colors

                fig = go.Figure()
                fig.add_trace(go.Bar(x=times, y=hvals, marker_color=bar_colors(hvals),
        ...
        with right:
            ...
            st.markdown(
                ...
                sig_row("Courroie tendue",   values.get("Courroie_accu_tendue", False),
                        "Yes — tensioned", "No") +
                sig_row("Courroie detendue", values.get("Courroie_accu_detendue", False),
                        "Yes — slack", "No") +
        ...
        st.markdown(f"<p style='text-align:center;color:#ccc;font-size:11px;margin-top:1.5rem;'>"
                    f"auto-refresh every {st.session_state.refresh_interval}s</p>",
                    unsafe_allow_html=True)
    ...
    with tab_data:
        ...
        if rows:
            ...
            st.dataframe(df.style.map(colour_state, subset=["state"]),
